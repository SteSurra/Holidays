...
Tutte le funzioni dell'app in dieci minuti di lettura.
Si apre da [stesurra.github.io/Holidays](https://stesurra.github.io/Holidays/).

**Prima di partire, installala.** In alto compare *Installa sul telefono*: da lì
diventa un'icona come le altre app, si apre a schermo intero e funziona **senza
rete**. Restano disponibili offline testi, schede, frasi, liste, valigia, note e
progressi; servono i dati solo per mappe, foto e meteo.

Non c'è registrazione: tutto quello che segni resta **sul tuo telefono**. Quindi
i progressi non si sincronizzano fra due telefoni, ognuno ha i suoi.
...
### «Sono a», il comando più utile dell'app
...
### I promemoria della giornata

Tre note scritte per quella città: **Andateci presto** (cosa si riempie a metà
mattina, cosa migliora di sera), **Da prenotare** (cosa vuole giorni di
anticipo), **Prima che chiuda** (mercati, botteghe, ultime corse).

Sotto, l'**ora del tramonto** calcolata dal telefono per quel giorno e quella
latitudine — funziona offline — con il posto da cui guardarlo.

Su **Miyajima** si aggiunge un riquadro sulle **maree**: sull'isola decidono se
il torii è nell'acqua o ci si cammina sotto, e cambiano ogni giorno. Il riquadro
spiega la regola e porta alla pagina dell'isola che segna le due fasce buone.
...
I cerchi numerati sono le tappe; i punti verdi i luoghi da vedere, e il simbolo
dice il tipo (torii per i santuari, tetto per i templi, montagna per la natura).
...
La guida contiene **tutte le possibilità**: 163 luoghi, nessuno li fa tutti. La
selezione serve a ridurre la mappa a ciò che ti interessa, così per strada non è
un tappeto di punti.

**Ogni scheda ha un quadratino** in alto, accanto al cuore:

- **spunta verde** → il luogo è sulla mappa;
- **quadratino vuoto** → il luogo è tolto dalla mappa.
...
Google Maps*: così puoi togliere un posto mentre lo stai guardando.

Per lavorare in blocco, apri **Filtri**: i pulsanti **Seleziona tutti** e
**Deseleziona tutti** agiscono **solo su ciò che il filtro sta mostrando**, e il
numero fra parentesi lo conferma. Filtrando su Kyoto, *Deseleziona tutti (35)*
toglie i 35 di Kyoto e lascia Tokyo intatta. Se sbagli, c'è *Annulla* nel
messaggio che compare in basso.

> **Il modo veloce di prepararsi.** Filtra su una città, *Deseleziona tutti*,
> poi rimetti i cinque o sei posti che ti interessano. Ripeti per ogni tappa:
> dieci minuti, e hai una mappa pulita per tutto il viaggio.

Se hai tolto qualcosa, sotto la mappa una riga lo dice — *«Sulla mappa ci sono …
luoghi di …: gli altri li hai nascosti»* — così un buco ha sempre una
spiegazione. Anche la riga sopra l'elenco porta due numeri: quanti luoghi ci
sono nel filtro e quanti di quelli stanno sulla mappa.

Le scelte restano salvate anche chiudendo l'app.
...
I due stati «spenti» non si confondono:

| Aspetto della scheda | Significato |
|---|---|
| Foto sotto un velo verde, timbro **✓ VISITATO** | Già visitato |
| Foto grigia, nessun timbro, quadratino vuoto | Tolto dalla mappa |
| Velo verde **e** quadratino vuoto | Visitato e tolto dalla mappa |

Sulla mappa i punti visitati **restano visibili** ma sbiaditi, e tengono il loro
simbolo di categoria con una spunta nell'angolo: continui a vedere *cosa* era
quel posto, non solo che ci sei stato.

### 2.3 · Disegnare un'area e farsi creare il giro

Serve a dire: *«sono in questa zona, ho tre ore, fammi un giro a piedi con
quello che c'è qui»*.

1. Porta la mappa sulla zona e premi **✎ Disegna un'area**.
2. Tieni il dito premuto e **disegna un cerchio** attorno alla zona. Non serve
   chiuderlo bene: basta un'area riconoscibile. Compare una linea tratteggiata.
3. Alza il dito: l'app calcola il giro.
4. Premi **Apri il giro in Google Maps**, oppure prima **◎ Parti da dove sono**
   per far partire il giro dalla tua posizione reale invece che da uno dei
   luoghi cerchiati.

Il messaggio che compare va letto tutto. Un esempio reale:

> *17 luoghi nell'area. 3 già visitati esclusi. […] Giro di 11 tappe, circa
> 5,2 km in linea d'aria. Ne ho lasciate fuori 6: Google Maps non accetta più di
> 9 tappe intermedie.*

- **«3 già visitati esclusi»** — i posti segnati come visitati **vengono buttati
  fuori dal giro**. Se ieri hai fatto Gion, oggi l'app non te lo rimette in
  mezzo. Sono contati a parte, così sai sempre quanti ne ha saltati e perché.
- **Anche i luoghi tolti dalla mappa non entrano**: se non sono sulla mappa, per
  il lazo non esistono.
- **L'ordine non è casuale**: l'app calcola il percorso più corto che li tocca
  tutti e poi raddrizza gli incroci. La distanza è in linea d'aria, quindi a
  piedi sarà un po' di più.
- **Il limite di 9 tappe** è di Google Maps, non dell'app. Se il cerchio prende
  troppo, disegna un'area più piccola.

Se nel cerchio finiscono **solo** posti già visitati, l'app non crea un giro
vuoto: te lo dice e suggerisce di allargare l'area o togliere qualche spunta.

Per uscire dal lazo senza disegnare, ripremi il pulsante.

### 2.4 · I livelli

*Luoghi da visitare* e *Hotel* sono accesi di partenza. Gli altri no:

- **Locali Tabelog** — ristoranti con il punteggio del sito giapponese di
...
## 3 · I dettagli di un luogo

Il pulsante **Dettagli ↗** apre la scheda completa: foto con attribuzione, nome
in italiano e in giapponese, e un testo che mette insieme cos'è il posto, quanto
tempo ci vuole, cosa stai guardando se è la prima volta che vedi una cosa del
genere e il consiglio specifico di quel luogo.

Sotto, sette sezioni fisse:

| Sezione | Contenuto |
|---|---|
| **Perché vale la visita** | Il motivo per cui è in guida |
| **Cosa osservare** | Dove guardare quando ci sei davanti |
| **Come viverlo** | Tempo indicativo e cosa sapere prima |
| **La chiave della città** | Il contesto storico della tappa |
| **Rispetto e buone maniere** | Come ci si comporta in quel tipo di luogo |
| **Come collegarlo al viaggio** | Come incastrarlo nella giornata |
| **Nota semiseria** | Una battuta, diversa per ognuno dei 163 luoghi |

**«Come viverlo» è la sezione da leggere prima di uscire di casa.** È dove sta
quello che fa perdere un viaggio se non lo sai: che il tempio dei ninja a
Kanazawa si visita **solo con prenotazione telefonica e in giapponese**; che il
Ruriko-in a Kyoto apre **poche settimane all'anno**; che i Giardini Higashi
Gyoen a Tokyo **chiudono lunedì e venerdì**; che dei nove bagni di Shibu Onsen
chi non dorme in paese **ne può usare uno solo**; che il To-ji ospita un grande
mercato **il 21 di ogni mese**.

In fondo: il **glossario** delle parole giapponesi usate nella scheda, le
**fonti** con i collegamenti agli enti ufficiali per controllare qualunque cosa,
e i pulsanti per segnare la visita, salvare fra i preferiti e aprire Google Maps.

---

## 4 · Scopri

Quattro sezioni con la stessa struttura della mappa: ricerca, filtri, schede,
*Dettagli*, cuore per salvare, pulsante per segnare.

- **Cibo** — 212 specialità per città e portata. Ogni scheda ha il nome
  giapponese **da mostrare al banco**, il contesto in cui si mangia e se è una
  scoperta locale o un classico. L'etichetta verde **La nostra scelta** segnala
...
In cima i numeri che si chiamano con un tocco: **110** polizia, **119** ambulanza
e pompieri, e la **Japan Visitor Hotline** del turismo giapponese, 24 ore su 24
in inglese.

Sotto, tenuti separati per non farti esitare quando serve l'ambulanza, i numeri
per tutto il resto: l'**Ambasciata d'Italia a Tokyo** con il cellulare di
servizio **fuori orario d'ufficio**, il **Consolato Generale a Osaka** che copre
le prime tappe, l'**Unità di Crisi della Farnesina** a Roma 24 ore su 24, la
ricerca **JNTO** di ospedali filtrabili per lingua parlata, e **AMDA**, che fa da
**interprete al telefono durante la visita medica**, gratis.

C'è anche **Trova le mie coordinate**, da leggere o mostrare a chi ti soccorre
...
sera è fresca. Due modi di usare la lista, a scelta:

- **la casella a sinistra** — spunta e via, per le cose che non si contano;
- **il campo numerico a destra** — scrivi **quante** ne stai mettendo. Scrivendo
  un numero l'oggetto si spunta da solo; svuotando il campo si despunta.
...
- **Progressi** — quanto hai visto, mangiato, comprato e letto, in totale e città
  per città. Da qui si azzera tutto.
- **Contanti** — monete e banconote **disegnate in scala reale**, con diametro,
  peso e cosa c'è sopra, per riconoscere al volo cosa hai in mano. Più il
...
Scrivendo un **numero** invece di una parola, viene convertito in euro al cambio
del giorno: comodo davanti a un cartellino del prezzo.

---

## Domande rapide

**Funziona senza rete?** Sì per testi, schede, frasi, liste, valigia, note e
progressi. No per mappe, foto e meteo. WC, fontanelle e konbini funzionano
offline **se li hai accesi almeno una volta** con la rete.

**Ho tolto un luogo dalla mappa per sbaglio.** Se è appena successo in blocco, c'è
*Annulla* nel messaggio in basso. Altrimenti ritrovi la scheda nell'elenco — c'è
sempre, anche se non è sulla mappa — e ripremi il quadratino.

**Ho segnato visitato per sbaglio.** Ripremi lo stesso pulsante.

**Perché un punto sulla mappa è sbiadito?** L'hai già visitato. Se invece è
sparito del tutto, l'hai tolto dalla mappa.

**Il giro in Google Maps ha meno tappe di quelle che ho cerchiato.** Google
accetta al massimo 9 tappe intermedie, e l'app salta i posti già visitati. Il
messaggio sotto la mappa dice sempre quanti ne ha lasciati fuori e per quale dei
due motivi.

**I miei progressi si vedono sul telefono dell'altro?** No: restano sul
dispositivo su cui sono stati segnati.

**Gli orari nelle schede sono garantiti?** No, e la guida lo dice. Orari, code e
stagioni cambiano: le schede danno tendenze e regole d'accesso, ma la fonte
ufficiale è sempre collegata in fondo alla scheda. Prima di un viaggio lungo per
una cosa sola, controlla lì.
