...
17. Verify JavaScript syntax, unique IDs, required fields, source coverage, distinct detail copy, mobile width at 320 and 390 px, mobile navigation grouping, filters, saved state, progress totals and reset, image rotation and fallback, attribution, popup photos and completion, marker deep links, wheel and touch map interaction, map resizing, service-worker cache version, repository cleanliness, exact lodging-name alignment between route and map, a detail link for every visit marker, every merchant rating carrying its source and date, saved selections restoring without touching other stops, and each saved entry expanding into its full card.
